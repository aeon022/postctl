...
`
	...
	if len(posts) != 4 {
		t.Fatalf("expected 4 posts (twitter, linkedin, threads, mastodon), got %d", len(posts))
	}
	...
	if !platforms["twitter"] || !platforms["linkedin"] || !platforms["threads"] || !platforms["mastodon"] {
